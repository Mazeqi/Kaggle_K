...
def train(args):
    ...
    model = network.ResNet18(num_class=10)
    if args.use_gpu:
        model = model.cuda()
    optimizer = optim.SGD(model.parameters(),lr=args.lr,momentum=args.momentum,weight_decay=args.weight_decay)
    criterion = nn.CrossEntropyLoss()
    trainer = solver(model, args.use_gpu)
...
def set_arg():
    ...
    parser.add_argument('--batch_size',type=int, default=32, help='trainng batch size')
    ...
    parser.add_argument('--train_class_dir',type=str,default='train_class_dir',help='the directory to save all data that have class')
